...
dat$elapsed.time <- mapvalues(dat$elapsed.time, from = 38.6, to = 38.7)
dat <- dat[! dat$elapsed.time == 187, ]

# adding a column with treatment names
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('2', '5', '9'), to = c(rep('AER25', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '6'), to = c(rep('AER20', 2)))
dat$treat <- mapvalues(dat$treat, from = c('1', '8'), to = c(rep('AER30', 2)))

dat$replicate <- dat$id
dat$replicate <- mapvalues(dat$replicate, from = c('1', '2', '4'), to = c(rep('1', 3)))
dat$replicate <- mapvalues(dat$replicate, from = c('5', '6', '8'), to = c(rep('2', 3)))
dat$replicate <- mapvalues(dat$replicate, from = c('9'), to = c(rep('3', 1)))
...
# outlet data 
dat <- dat[dat$treat == 'AER25' | dat$treat == 'AER20'| dat$treat == 'AER30', ]
...
# As the backgrounds are very similar an average of the background tube for tunnel 2, 5 and 7 is used as the back
# ground value. 
...
dat$air.flow <- mapvalues(dat$air.flow, from = 'AER30', to = 40.10 * 60)
dat$air.flow <- mapvalues(dat$air.flow, from = 'AER20', to = 26.76 * 60);
